...
func TestDemoWorkspaceNavigationAndPersistence(t *testing.T) {
	...
	err = chromedp.Run(ctx,
		...
		chromedp.Click("#logs-regex-toggle", chromedp.ByQuery),
		chromedp.Poll(`document.querySelector('#logs-regex-toggle')?.getAttribute('aria-pressed') === 'true'`, nil, chromedp.WithPollingTimeout(2*time.Second)),
		chromedp.Click("#logs-regex-toggle", chromedp.ByQuery),
		chromedp.Poll(`document.querySelector('#logs-regex-toggle')?.getAttribute('aria-pressed') === 'false'`, nil, chromedp.WithPollingTimeout(2*time.Second)),
		chromedp.Focus("#logs-search", chromedp.ByQuery),
		chromedp.Evaluate(`(() => { const input = document.querySelector('#logs-search'); input.value = 'e'; input.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true })); })()`, nil),
		chromedp.Poll(`document.querySelector('#logs-match-status')?.textContent === '1 OF 3'`, nil, chromedp.WithPollingTimeout(3*time.Second)),
		chromedp.Evaluate(`document.querySelector('#logs-search').dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true }))`, nil),
		chromedp.Poll(`document.querySelector('#logs-match-status')?.textContent === '2 OF 3'`, nil, chromedp.WithPollingTimeout(2*time.Second)),
		chromedp.Evaluate(`document.querySelector('#logs-search').dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', shiftKey: true, bubbles: true }))`, nil),
		chromedp.Poll(`document.querySelector('#logs-match-status')?.textContent === '1 OF 3'`, nil, chromedp.WithPollingTimeout(2*time.Second)),
